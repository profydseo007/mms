...
type IconName = 'Clock' | 'DollarSign' | 'Star' | 'CheckCircle';

function LigatureIcon({ name, className }: { name: IconName; className?: string }) {
  ...
  return <span aria-hidden="true" className={className}>{symbols[name]}</span>;
}
...
export default function UKScienceTutorPage({ onNavigate }: SubjectComponentProps) {
  return (
