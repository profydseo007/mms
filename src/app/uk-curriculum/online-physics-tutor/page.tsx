...
interface LigatureIconProps {
  name: 'Clock' | 'DollarSign' | 'Star' | 'CheckCircle';
  className?: string;
}

const LigatureIcon = ({ name, className = '' }: LigatureIconProps) => {
  const icon = name === 'Clock' ? '◷' : name === 'DollarSign' ? '$' : name === 'Star' ? '★' : '✓';
...
export default function UKPhysicsTutorPage({ onNavigate }: SubjectComponentProps) {
  return (
