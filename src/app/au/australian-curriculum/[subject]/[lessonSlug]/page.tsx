...
const SUBJECTS: Record<string, LessonPlanSubject> = {
...
export default function AustralianLessonPlanPage() {
  const { subject, lessonSlug } = useParams<{ subject: string; lessonSlug: string }>();
  const yearWord = lessonSlug.match(/^lesson-plan-for-year-(.+)$/)?.[1];
  const year = yearWord ? YEARS[yearWord] : undefined;
  const lessonSubject = SUBJECTS[subject];
  if (!lessonSubject || !year) return null;
  return <CurriculumLessonPlan region="Australian" subject={lessonSubject} year={year} />;
}