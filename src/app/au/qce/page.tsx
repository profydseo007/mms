      ...
      <section className="py-24 bg-soft-gray">
        <div className="container mx-auto px-margin-mobile md:px-margin-desktop max-w-max-width">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { icon: 'assignment', color: 'bg-primary-fixed text-royal-purple', label: 'Internal Assessment', text: '3 school-based tasks per subject (75% of overall result). We prepare for every IA type including research investigations, problem-solving tasks, and extended responses.' },
              { icon: 'quiz', color: 'bg-secondary-container text-secondary', label: 'External Examination', text: 'QCAA-set exams in October–November (25% of overall result). We use official QCAA past papers and marking guides to maximise external marks.' },
              { icon: 'bar_chart', color: 'bg-lime-green/20 text-secondary', label: 'Overall Position (OP)', text: 'Subject result scaled to a position between 1 (highest) and 25. Scaling depends on subject cohort performance — we advise on subject selection.' },
              { icon: 'school', color: 'bg-royal-purple/10 text-royal-purple', label: 'ATAR', text: 'Queensland ATAR calculated from best five scaled results. We focus preparation where the ATAR impact is greatest for each individual student.' },
            ].map((item) => (
              <div key={item.label} className="bg-white p-6 rounded-2xl shadow-ambient">
                <div className={`w-12 h-12 ${item.color} rounded-xl flex items-center justify-center mb-4`}><span className="material-symbols-outlined">{item.icon}</span></div>
      ...
      <section className="py-24 bg-white">
        <div className="container mx-auto px-margin-mobile md:px-margin-desktop max-w-max-width">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              { icon: 'functions', title: 'Maths', subs: ['General Mathematics', 'Mathematical Methods', 'Specialist Mathematics', 'Essential Mathematics'] },
              { icon: 'auto_stories', title: 'English', subs: ['English', 'Literature', 'English & Literature Extension', 'Essential English'] },
              { icon: 'science', title: 'Sciences', subs: ['Biology', 'Chemistry', 'Physics', 'Earth & Environmental Science'] },
              { icon: 'history_edu', title: 'Humanities', subs: ['Modern History', 'Ancient History', 'Geography', 'Legal Studies'] },
              { icon: 'business_center', title: 'Business', subs: ['Accounting', 'Business', 'Economics', 'Tourism'] },
              { icon: 'psychology', title: 'Social Science', subs: ['Psychology', 'Sociology', 'Philosophy & Reason', 'Health'] },
            ].map((subject) => (
        ...
        <div className="container mx-auto px-margin-mobile md:px-margin-desktop max-w-max-width relative z-10">
          <div className="bg-white rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col lg:flex-row">
            <div className="lg:w-1/2 p-12 lg:p-20 space-y-8 bg-surface">
              ...
              <div className="space-y-4">{['QCAA syllabus-matched', 'IA & external exam prep', 'All General subjects covered'].map((pt) => <div key={pt} className="flex items-center gap-4"><span className="material-symbols-outlined text-royal-purple">check_circle</span><span className="text-charcoal font-headline-md font-bold">{pt}</span></div>)}</div>
            </div>
